...
Pi 0.81.1 was installed when Calm was first built, with later reverification on Pi 0.82.0 and 0.84.1.
The inspected Pi CHANGELOG through 0.82.0 showed no relevant presentation API introduction, and all three versions remain verification evidence rather than compatibility bounds.
...
The fix installs one idempotent presentation adapter, verified on Pi 0.81.1, 0.82.0, and 0.84.1, on the exported `AssistantMessageComponent.updateContent` method.
...
The fix installs a separate idempotent presentation adapter, verified on Pi 0.81.1, 0.82.0, and 0.84.1, on the exported `InteractiveMode.addMessageToChat` method.
The adapter probes for that exact method and, per the [compatibility contract](calm.md#pi-compatibility), degrades independently with a diagnostic rather than gating on a version number.
...
| Policy class | Pi transcript path | Calm result (verified on Pi 0.81.1, 0.82.0, and 0.84.1) |
| --- | --- | --- |
...
| `synthetic-user` | Firstmate extension `sendUserMessage`, terminal-injected input, Firstmate-generated Pi positional brief, or the already non-displayed session-start nudge | Canonically classified text-only operational user messages stay ordinary semantic user messages but render through the zero-height adapter (verified on Pi 0.81.1, 0.82.0, and 0.84.1) under Calm; legacy entries stay gaplessly controllable, and the session-start nudge retains its existing non-displayed custom-message path. |
...
`tests/fm-pi-primary-types.test.sh` performs strict no-emit TypeScript checking against the installed Pi declarations, currently package version 0.84.1.
