...
`bin/fm-sessionstart-nudge.sh` is the shared nudge command every harness transport ultimately invokes.
...
It sources `bin/fm-primary-scope-lib.sh`, while Pi and OpenCode use `bin/fm-primary-scope.sh` as the executable boundary before registering primary handlers.
Both paths use the checkout that supplied the tracked integration, so a linked task worktree stays silent even when primary-home environment overrides point at a genuine primary's state.
...
`tests/fm-sessionstart-nudge.test.sh` proves wrapper silence for both gate signals, an unmarked linked worktree with either local or inherited primary-home paths, a missing state directory, and an already-owned lock.
