...
| `fm-contender.sh`        | Retain finished data-only scout reports through durable contender pick and settlement cleanup |
...
| `fm-primary-scope.sh`    | Expose the shared primary-home predicate to tracked non-shell integrations           |
...
| `fm-secondmate-report.sh` | Serializes parent status appends and correlated or document-pointer reports           |
| `fm-procevent-board-reply.sh` | Run the mission control board reply service, arm its wake, post firstmate's replies into the board conversation, normalize captain board requests, and apply board answers through the decision lifecycle ([`docs/mission-control.md`](mission-control.md)) |
...
| `fm-worktree-ownership-lib.sh` | Shared fail-closed ownership check for Treehouse-backed task worktrees ([`architecture.md`](architecture.md#worktrees-not-branches-in-your-checkout)) |
