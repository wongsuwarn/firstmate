...
Claude's Stop `asyncRewake` hook and Grok's Stop auto-arm hook own tokenless re-arm cycles, Codex uses bounded foreground checkpoints, Pi and pi-signed use the same two tracked primary extensions, and OpenCode uses its TUI plugin.
...
Bootstrap also reports a `MAIN_DIVERGED:` line when `FM_ROOT` is on its default branch and that branch carries commits `origin/<default>` does not, which is the state fast-forward self-update can no longer reconcile; follow the printed inspect-and-reconcile remediation rather than treating it as an installable tool problem, and note the check function never fetches, so a local-only or offline install stays silent.
In a read-only session that did not get the fleet lock, the same line is advisory and omits the printed fetch instruction.
...
FM_ARM_ATTACH_POLL=0.5  # seconds between checks while fm-watch-arm waits on an orphaned healthy watcher or restart peer
...
FM_STALE_ESCALATE_SECS=240         # idle seconds after provably-working normalized stale pane content is first absorbed before it escalates; recognized footer quantity drift does not restart this timer, while stale panes whose crew is not provably working surface immediately unless they declare the pause verb
...
FM_WEDGE_DEMAND_INSPECT_COUNT=3    # consecutive provably-working stale escalations on the same unchanged normalized pane content before demand-deep-inspection is added
