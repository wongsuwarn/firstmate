...
Every firstmate session start now fast-forwards the primary checkout to close that gap on its own, so a merge reaches the board without the captain running an update by hand; a checkout that cannot advance (local changes, or history diverged from origin) is left untouched and reported instead, see `bin/fm-bootstrap.sh`'s header comment for the exact mechanism.
