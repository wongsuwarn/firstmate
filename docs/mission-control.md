...
Every firstmate session start, and firstmate merging its own repo's PR, now fast-forward the primary checkout to close that gap on their own, so a merge reaches the board without the captain running an update by hand and without waiting for the next session to start; a checkout that cannot advance (local changes, or history diverged from origin) is left untouched and reported instead, see `bin/fm-ff-lib.sh`'s `primary_self_update` header comment for the exact mechanism shared by both triggers.
