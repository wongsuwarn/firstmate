...
`bin/fm-supervision-lib.sh` owns that need predicate and excludes a persistent `kind=secondmate` record only after cheap durable local parent evidence shows a readable, parseable resting reconciliation with no open keyed activity, decision, blocker, or pending reply; missing or ambiguous status and malformed or unreadable pending-reply evidence remain watched.
