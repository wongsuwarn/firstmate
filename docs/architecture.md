...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or an X-mode mention, stale panes whose crew is not provably working and whose current terminal status has not already surfaced, a new terminal status on an otherwise unchanged stale pane, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
Repeated provably-working stale escalations on the same unchanged normalized pane content add an escalation count to the wake reason and, at `FM_WEDGE_DEMAND_INSPECT_COUNT`, a `demand-deep-inspection` marker.
...
Fresh normalized stale content uses the same current-state read before trusting the status log, so an active run or a proven busy worker outranks an old captain-relevant status-log line left behind before validation.
After a terminal stale status surfaces, the watcher keeps observing the task but deduplicates that exact current status line against the durable marker written by `mark_surfaced`; a different terminal status surfaces immediately even when the normalized pane content remains unchanged.
...
Code identity is commit identity, so a crew that resumed after its run reported ready keeps that run attributed while its new work is still uncommitted.
A ready verdict is therefore demoted whenever the worktree holds unlanded uncommitted work, to working when the crew is busy and to unknown when it is not, because only the second of those stays absorb-ineligible for a crew that wedged with unlanded work.
The script header owns the exact run-head ancestry rules and that demotion contract.
During no-mistakes' `ci` monitor phase, it also reads the ci step log tail because `axi status` reports both "still waiting on checks" and "checks green, waiting on merge" as `ci,running`.
The most recent recognized ci log marker wins, so checks-green monitoring reports done while a later re-arm, failed-check, or issue marker returns the crew to working.
A ci log that was read but matched no recognized marker is treated as unconfirmable rather than as agreement, so the crew's own earlier checks-green report is no longer trusted once that vocabulary stops matching; an absent ci log is a different case and still trusts the report.
...
A pure informational question uses the narrowest read that can answer it and does not justify a whole-fleet snapshot.
Unchanged open decisions remain durable but are not re-narrated on every otherwise unchanged notification.
...
That block owns the live wait shape for the running primary harness: Claude's Stop `asyncRewake` hook and Grok's Stop auto-arm hook own tokenless re-arm cycles, Codex uses bounded foreground checkpoints, Pi and pi-signed use the same two tracked primary extensions, and OpenCode uses its TUI plugin.
...
Grok's `bin/fm-grok-stop-autoarm.sh` hook applies the same primary-scope, session-lock, AFK, and supervision-need boundaries, keeps one home-scoped arm in its foreground process tree, and translates actionable closes into exit-2 feedback.
[`watcher-continuity.md`](watcher-continuity.md) owns the Stop auto-arms' residual active-turn coverage, Grok successor verification, and watcher-status command-gating boundary.
...
`bin/fm-supervision-lib.sh` owns that shared need predicate; [`turnend-guard.md`](turnend-guard.md) owns its current secondmate-idleness and fail-closed evidence contract.
...
The always-on watcher also uses that library's absorb classification on no-verb signals and first-sighting normalized stale pane content before status-log terminality is trusted, while the daemon maintains distinct wedge and declared-pause recheck cadences.
