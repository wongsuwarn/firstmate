...
`bin/fm-supervision-lib.sh` owns that need predicate and excludes a persistent `kind=secondmate` record only after its local parent status has a parseable resting reconciliation with no open keyed activity or decision and no open pending reply; missing or ambiguous evidence remains watched.
