...
A tracked integration supplies its own checkout as the scope root; `FM_ROOT_OVERRIDE`, `FM_HOME`, and `FM_STATE_OVERRIDE` may select runtime and state paths but cannot substitute another checkout's identity.
...
A persistent `kind=secondmate` record is excluded only when cheap durable local parent evidence positively proves it is resting: its status is readable and parseable, its latest durable verb is `done`, `resolved`, or `captain-held`, it has no open keyed activity under `bin/fm-classify-lib.sh`, and it has no open parent pending reply.
When a terminal status follows leftover open decisions or blockers, a mutating supervision check treats those folds as process debt, appends the matching keyed or bare `resolved` records under the status append lock, and re-folds before deciding whether the secondmate is idle.
This healing is local to the supervision predicate and does not change `status_open_decisions` for other consumers; a read-only check leaves the debt open and keeps the secondmate supervised.
Latest `working`, `needs-decision`, `blocked`, `failed`, or `paused` status, open keyed activity, and open parent pending replies remain supervised.
Canonical routed status prefixes may carry one decision key token and one 16-hex correlation token in either order; malformed or duplicate tokens keep the secondmate supervised.
...
`tests/fm-turnend-guard.test.sh` covers the supervision predicate's ordinary metadata, positively idle secondmate exclusion including canonical correlation tokens, live decision, blocker, failure, activity, and parent-reply supervision, terminal self-healing with durable keyed and bare resolutions, read-only behavior, concurrent append preservation, fail-closed status and pending-reply evidence, process-event and X-mode sources, main and secondmate primary scope, child-worktree exclusion even with inherited primary-home overrides, `FM_HOME` and `FM_STATE_OVERRIDE` precedence, the live-lock and fresh-beacon guard predicate, the cooperative `--claude` claim wait, monotonic failed-epoch progression, bounded attended fail-open, post-alarm continuation suppression, positive recovery reset, Pi logical-run latching, missing-`jq` behavior, all five primary registrations, Grok native and legacy selection, typed field precedence, malformed input, and exactly-one-path safety.
