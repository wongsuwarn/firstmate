...
On 2026-08-23, gh 2.97.0 was verified in a scratch Git repository carrying `origin=https://github.com/wongsuwarn/firstmate.git` and the third-party upstream remote.
...
`bin/fm-pr-create-pretool-check.sh` scopes that classifier to a checkout whose origin is `wongsuwarn/firstmate` and renders the existing harness-specific deny responses.
...
It proves bare and wrong-target PR creation are denied, explicit correct-target creation is allowed, unrelated commands are allowed, malformed PR creation is refused, and binding application is idempotent.
