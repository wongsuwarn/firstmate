...
fm_worktree_claimed_by_other_task() {
  local state=$1 self=$2 worktree=$3 target meta id line claims claim claim_real
  ...
  if ! target=$(CDPATH='' cd -- "$worktree" 2>/dev/null && pwd -P); then
    FM_WORKTREE_OWNERSHIP_REASON="worktree cannot be resolved: $worktree"
    return 2
  fi
