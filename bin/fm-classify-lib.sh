...
# and first sighting of normalized stale pane content, never on every wake, so
# the per-wake triage stays cheap. status_open_decisions_incremental (see "incremental (cursor-backed)
...
# bracket token before the colon so the leading word is recovered cleanly.
# The stricter secondmate-idle reader validates the permitted key/corr tokens.
