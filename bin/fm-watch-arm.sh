...
# own tracked background task (e.g. run_in_background), or inside a primary Stop
# auto-arm hook's foreground process tree (`bin/fm-claude-stop-autoarm.sh` or
# `bin/fm-grok-stop-autoarm.sh`), where the harness owns the process group and the
# hook's exit-2 feedback is the notification. Run it as its own standalone
