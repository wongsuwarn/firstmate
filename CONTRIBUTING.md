...
`bin/fm-brief.sh` detects firstmate-repository ship and scout targets by Git top-level or normalized `origin` identity and inserts a worker identity guard before the task.
