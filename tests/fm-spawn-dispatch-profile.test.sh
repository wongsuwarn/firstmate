...
test_no_profile_keeps_claude_profile_defaults() {
  ...
  # Every launch command carries this task's own browser-session binding and the
  # guard shim's PATH entry (docs/browser-session-isolation.md).
  expected="CHROME_DEVTOOLS_AXI_SESSION='$id' PATH='${ROOT}/bin/shims':\$PATH CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false claude --dangerously-skip-permissions \"\$('${ROOT}/bin/fm-operational-input.sh' encode launch-brief < '$HOME_DIR/data/$id/brief.md')\""
...
test_active_dispatch_profile_allows_raw_launch_command() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  # The raw-command escape hatch is still a launched agent, so it carries the
  # same browser-session binding as every verified adapter.
  [ "$launch" = "CHROME_DEVTOOLS_AXI_SESSION='$id' PATH='${ROOT}/bin/shims':\$PATH custom-agent --flag" ] \
